...
function genNewChart(data, colors) {
    ...
    return new Chart(ctx, {
        ...
        options: {
            ...
            onClick: (e) => {
                let points = globalMainChart.getElementsAtEventForMode(e, 'nearest', {
                    intersect: true
                }, true);

                if (points.length) {
                    let point = points[0];
                    let label = globalMainChart.data.labels[point.index];
                    let value = globalMainChart.data.datasets[point.datasetIndex].data[point.index];
