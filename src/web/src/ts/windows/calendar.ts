...
export function cycle(configOptions: { end: Date, start: Date }, monthOffset = 0) {
    const cellHolder = $("#week-cell-holder");
    const dateValues = getDateValues(configOptions, monthOffset);
    ...
    dateValues.weeks.forEach((week) => {
        let newEndWeek = new Date();
        if (dateValues.days[week[1]] === undefined) {
            newEndWeek.setDate(newEndWeek.getDate() + (week[1] + 1) - dateValues.days.length)
        }

        cellHolder.append(`<div class="cell-week">${moment(new Date(dateValues.days[week[0]])).format("MMM Do")} - ${moment(dateValues.days[week[1]] === undefined ? newEndWeek : new Date(dateValues.days[week[1]])).format("MMM Do")}</div>`);
    });

    return dateValues.limitReached;
}
...
function getDateValues({ end, start }: { end: Date, start: Date }, monthOffset = 0) {
    ...
    for (let i = today.getDay(); i > 0; i--) {
        ...
        if (lastMonth.getFullYear() === start.getFullYear()
            && lastMonth.getMonth() === start.getMonth() &&
            start.getDate() > ((lastMonth.getDate() - i) + 1)) {
    ...
    for (let i = 1; i <= currentMonth.getDate(); i++) {
        if (currentMonth.getFullYear() === end.getFullYear()
        ...
        if (lastMonth.getFullYear() === start.getFullYear()
            && lastMonth.getMonth() === start.getMonth() &&
            start.getDate() < lastMonth.getDate()) {
            limitReached.backwards = true;
            allowNext = false;
        } else {
            allowNext = true;
        ...
        if (limitReached.forwards) { break };
    ...
    for (let i = 1; i <= (7 - currentMonth.getDay()) - 1; i++) {
        if (nextMonth.getFullYear() === end.getFullYear()
    ...
    console.log({ weeks, days, limitReached });
